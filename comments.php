...
<p class="mdx-comment-login-needed mdui-typo"><?php printf(__('你需要 <a href="%s">登录</a> 后才能发表评论.','mdx'), get_option('siteurl')."/wp-login.php?redirect_to=".urlencode(get_permalink()));?></p>
<?php else:
...
<?php $defaults = array(
		...
		'label_submit'=>__('提交','mdx'),
		'comment_notes_after'=>'',
		'id_form'=>'commentform',
		'cancel_reply_link'=>__('取消回复', 'mdx'),
		'comment_field'=>'<div class="mdui-textfield mdui-textfield-floating-label'.$mdx_emj_cla.'"><i class="mdui-icon material-icons">textsms</i><label class="mdui-textfield-label">'.__('评论...','mdx').'</label><textarea class="mdui-textfield-input" name="comment" id="comment"></textarea></div>'.$mdx_emj_ele.$mdx_alu,
		'fields'=>apply_filters('comment_form_default_fields', array(
				...
				'url'=>'<div class="mdui-textfield mdui-textfield-floating-label commurl"><i class="mdui-icon material-icons">&#xe157;</i><label class="mdui-textfield-label">'.__('网站','mdx').'</label><input class="mdui-textfield-input" type="url" id="url" name="url" value="'.esc_attr($comment_author_url).'"></div>',
		)));
